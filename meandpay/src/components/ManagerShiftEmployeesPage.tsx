...
interface ManagerShiftEmployeesPageProps {
  onBack: () => void;
}
...
/* ─── Main Component ─────────────────────────────────────── */
export function ManagerShiftEmployeesPage({ onBack }: ManagerShiftEmployeesPageProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [shiftRes, empRes, mappingRes] = await Promise.all([
        ...
        fetch(`${BASE_URL}/users/subordinates`, { headers }),
        fetch(`${BASE_URL}/mapping-shifts/my-team`, { headers }),
      ]);
